...
public sealed class LtxAudioWindowMaskerTests
{
    ...
    public void Audio_retake_window_seconds_are_deterministic()
    {
        ...
        LtxAudioWindowMasker.AudioMaskWindow w =
            LtxAudioWindowMasker.ComputeRetakeWindow(new RetakePlan(24, 24, 1.0), fps: 24, clipFrames: 97);
        ...
        LtxAudioWindowMasker.AudioMaskWindow clamped =
            LtxAudioWindowMasker.ComputeRetakeWindow(new RetakePlan(48, 999, 1.0), fps: 24, clipFrames: 96);
        Assert.Equal(41.0 / 24, clamped.StartTime, 6);
        Assert.Equal(89.0 / 24, clamped.EndTime, 6);

        // Zero-length retake => empty (no windowing).
        Assert.True(LtxAudioWindowMasker.ComputeRetakeWindow(new RetakePlan(0, 0, 1.0), 24, 97).IsEmpty);
    }
}
