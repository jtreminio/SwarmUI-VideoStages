...
public class TimelineAudioSegmentPlanCompilerTests
{
    ...
    public void Boundary_trims_partition_segments_and_advance_source_time()
    {
        ...
        AudioSegmentItemPlan[] items = result.Clips
            .Select(clip => Assert.Single(clip.Audio.Segments.Items))
            .ToArray();
    ...
    public void Duplicate_segment_id_keeps_the_first_usable_segment()
    {
        ...
        Assert.Equal(2, Assert.Single(result.Clips).Audio.Segments.Items[0].TrimStartSeconds);
        Assert.Contains(result.Diagnostics, diagnostic =>
    ...
    {
        VideoExecutionPlan plan = Plan(Clip(0));
        TimelineAudioSegmentSpec segment = Segment("voice", 0, 1) with
        {
        ...
        AudioSegmentItemPlan item = Assert.Single(
            Assert.Single(result.Clips).Audio.Segments.Items);
    ...
    public void Empty_first_segment_still_claims_its_duplicate_id()
    {
        ...
        Assert.Empty(Assert.Single(result.Clips).Audio.Segments.Items);
        Assert.Contains(result.Diagnostics, diagnostic =>
    ...
    private static void AssertWarning(
        TimelineAudioSegmentCompilation result,
    ...
    private static TimelineAudioSegmentSpec Segment(
        string id,
        double start,
        double length,
