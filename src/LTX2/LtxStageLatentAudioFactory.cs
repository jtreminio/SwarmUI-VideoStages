    ...
    LtxStageRuntimeSettings runtimeSettings)
{
    ...
    internal JArray TryResolveControlNetLengthFrames(AudioLengthPlan length)
    {
        if (length.Owner != AudioLengthOwner.ControlNet)
        {
            return null;
        }
        if (length.ControlNetSourceIndex is not int sourceIndex)
        {
