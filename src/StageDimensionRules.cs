...
internal static class StageDimensionRules
{
        ...
        int height)
    {
        ...
        int targetWidth = AlignTo16((int)Math.Round(Math.Max(width, 16) * stage.Upscale.Factor));
        int targetHeight = AlignTo16((int)Math.Round(Math.Max(height, 16) * stage.Upscale.Factor));
    ...
    {
        ArgumentNullException.ThrowIfNull(stage);
        int multiple = 32 * IcLoraApplicator.MaxKnownIcLoraDownscaleFactor(stage.IcLoras);
