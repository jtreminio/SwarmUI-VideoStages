...
const rootDefaults = (fps: number, frames = 24): RootDefaults =>
    ({ fps, frames }) as unknown as RootDefaults;
...
const clip = (duration: number, refs: RefImage[] = []): Clip =>
    ({ duration, refs, stages: [] }) as unknown as Clip;
...
describe("pxToFrame", () => {
    ...
    it("maps x across the region to frame = round(time*fps) from the start", () => {
        expect(pxToFrame(50, WIDTH, DUR, FPS, false)).toBe(60); // half → 2.5s → 60f
        expect(pxToFrame(25, WIDTH, DUR, FPS, false)).toBe(30); // quarter → 1.25s → 30f
    });

    it("measures from the clip end when fromEnd", () => {
        // Same x, but the frame is the distance from the end: 50% → 2.5s from end → 60f.
        expect(pxToFrame(50, WIDTH, DUR, FPS, true)).toBe(60);
        // Region left edge with fromEnd = the full clip away from the end.
        expect(pxToFrame(0, WIDTH, DUR, FPS, true)).toBe(120);
    });

    it("clamps at the low end to REF_FRAME_MIN", () => {
        // Region left edge, start-relative → frame 0 → floored to REF_FRAME_MIN (1).
        expect(pxToFrame(0, WIDTH, DUR, FPS, false)).toBe(1);
        // Region right edge, fromEnd → 0 frames from the end → floored to REF_FRAME_MIN (1).
        expect(pxToFrame(WIDTH, WIDTH, DUR, FPS, true)).toBe(1);
        expect(pxToFrame(-40, WIDTH, DUR, FPS, false)).toBe(1);
    });

    it("clamps at the high end to framesForClip's max", () => {
        // Right edge, start-relative → 5s → 120f (within the 121 max).
        expect(pxToFrame(WIDTH, WIDTH, DUR, FPS, false)).toBe(120);
        expect(pxToFrame(400, WIDTH, DUR, FPS, false)).toBe(120);
    });

    it("collapses degenerate width/px to REF_FRAME_MIN", () => {
        expect(pxToFrame(50, 0, DUR, FPS, false)).toBe(1);
        expect(pxToFrame(Number.NaN, WIDTH, DUR, FPS, false)).toBe(1);
        expect(pxToFrame(50, Number.NaN, DUR, FPS, false)).toBe(1);
    });
});

describe("clampClipRefsToDuration", () => {
    it("clamps ref frames past the new duration's max down to the max", () => {
        // duration 1.8s @24fps → getReferenceFrameMax = 49 (see framesForClip).
