...
describe("createTimelineDetailStrip", () => {
    ...
    it("zeros add and replace weights on stages rejected by an architecture rule", async () => {
        ...
        expect(savedClips(saveSpy)[0].stages[0].loraWeights).toEqual([1]);
        expect(savedClips(saveSpy)[0].stages[1].loraWeights).toEqual([0]);
        expect(savedClips(saveSpy)[0].stages[2].loraWeights).toEqual([1]);

        const select = document.querySelector<HTMLSelectElement>(
            ".vst-detail .vst-clip-lora-entry select",
        );
        if (!select) throw new Error("clip LoRA model select missing");
        select.value = "lora-y.safetensors";
        select.dispatchEvent(new Event("change", { bubbles: true }));

        expect(savedClips(saveSpy)[0].stages[0].loraWeights).toEqual([0.65]);
        expect(savedClips(saveSpy)[0].stages[1].loraWeights).toEqual([0]);
        expect(savedClips(saveSpy)[0].stages[2].loraWeights).toEqual([0.65]);
    });

    it("rebuilds stage LoRA policy only when live Control crosses its rule threshold", async () => {
        const reason =
        ...
        setup([
            {
                ...
                stages: [
                    {
                        control: 0.5,
                        loras: [{ name: "lora-x.safetensors", weight: 0.4 }],
                    },
                    {
                        control: 0.5,
        ...
        const loraInput = (): HTMLInputElement | null =>
            document.querySelector<HTMLInputElement>(
                '.vst-detail input[data-vst-focus-key="lora-weight-0"]',
            );
        const repair = (): HTMLButtonElement | null =>
            document.querySelector<HTMLButtonElement>(
                ".vst-reset-unsupported-stage-loras",
            );
        const setControl = (value: number): void => {
            const input = sliderNumberByLabel("Control");
            input.value = `${value}`;
            input.dispatchEvent(new Event("input", { bubbles: true }));
            jest.advanceTimersByTime(200);
        };

        const initialLoraInput = loraInput();
        expect(initialLoraInput?.value).toBe("0.7");
        expect(initialLoraInput?.disabled).toBe(false);
        setControl(0.4);
        expect(loraInput()).toBe(initialLoraInput);
        setControl(0);

        expect(savedClips(saveSpy)[0].stages[1].control).toBe(0);
        expect(loraInput()?.value).toBe("0.7");
        expect(loraInput()?.disabled).toBe(true);
        expect(
            document.querySelector(".vst-capability-notice")?.textContent,
        ).toBe(reason);
        ...
        setControl(0.5);
        expect(loraInput()?.disabled).toBe(false);
        expect(repair()).toBeNull();

        setControl(0);
        repair()?.click();
        const repaired = committedClips()[0];
        expect(repaired.loras).toEqual([{ name: "lora-x.safetensors" }]);
        expect(repaired.stages[0].loraWeights).toEqual([0.4]);
        expect(repaired.stages[1].loraWeights).toEqual([0]);
    });
